...
if any(~isfinite(heading_quality.Correlation) | ...
    ...
    warning(['At least one run has weak heading/yaw-rate consistency. ' ...
        'Inspect visualize_raw_data.m before interpreting heading errors.']);
end
...
layout = tiledlayout(3,2,'TileSpacing','compact','Padding','compact');
legend_handles = plotBinned(nexttile(layout),result.errors_vy, ...
    vx_bin,vx_centers, ...
...
shared_legend = legend(legend_handles,names, ...
    'Location','southoutside','Orientation','horizontal', ...
    'NumColumns',min(numel(names),5));
...
function line_handles = plotBinned(ax,error_cells,bins,centers,count,min_count, ...
        names,colors,xlab,ylab,ttl,cfg)
hold(ax,'on'); grid(ax,'on');
all_values = nan(numel(names),numel(centers));
line_handles = gobjects(numel(names),1);
for m = 1:numel(names)
    ...
    line_handles(m) = plot(ax,centers,values,style,'Color',colors(m,:), ...
...
layout = tiledlayout(3,2,'TileSpacing','compact','Padding','compact');
metrics = {'RunRMSE_vy','SampleRMSE_vy', ...
    'RunRMSE_r','SampleRMSE_r', ...
    'RunRMSE_psi','SampleRMSE_psi'};
titles = {'Run-weighted v_y','Sample-weighted v_y', ...
    'Run-weighted r','Sample-weighted r', ...
    'Run-weighted heading','Sample-weighted heading'};
ylabs = {'RMSE [m/s]','RMSE [m/s]', ...
    'RMSE [rad/s]','RMSE [rad/s]', ...
    'RMSE [rad]','RMSE [rad]'};
for tile = 1:6
    ax = nexttile(layout); hold(ax,'on'); grid(ax,'on');
    for m = 1:numel(names)
...
ax_vy = nexttile(layout);
plotAggregationTile(ax_vy,focus,names,colors,'vy', ...
    'v_y RMSE [m/s]',cfg);
title(ax_vy,'Lateral-velocity prediction error');
ax_r = nexttile(layout);
plotAggregationTile(ax_r,focus,names,colors,'r', ...
    'r RMSE [rad/s]',cfg);
title(ax_r,'Yaw-rate prediction error');
ax_psi = nexttile(layout);
plotHeadingAggregationTile(ax_psi,focus,names,colors,cfg);
title(ax_psi,'Heading prediction error');

legend_handles = gobjects(3+numel(names),1);
legend_handles(1) = plot(ax_vy,nan,nan,'s','Color',[.35 .35 .35], ...
    'MarkerFaceColor',[.65 .65 .65],'MarkerSize',7);
legend_handles(2) = plot(ax_vy,nan,nan,'ko','MarkerFaceColor','w', ...
    'MarkerSize',7);
legend_handles(3) = plot(ax_vy,nan,nan,'kd', ...
    'MarkerFaceColor',[.8 .8 .8],'MarkerSize',7);
for m = 1:numel(names)
    legend_handles(3+m) = plot(ax_vy,nan,nan,'-', ...
        'Color',colors(m,:),'LineWidth',4);
end
legend_labels = ["Sample-weighted";"Run-weighted"; ...
    "Declared-lap weighted";string(names(:))];
shared_legend = legend(legend_handles,legend_labels, ...
    'Orientation','horizontal','NumColumns',min(6,numel(legend_labels)));
shared_legend.Layout.Tile = 'south';
title(layout,figure_title);
end
...
[frozen_line,adaptive_line] = plotAdaptiveCurve(nexttile(layout), ...
    frozen_prop,adaptive_prop,color, ...
...
shared_legend = legend([frozen_line,adaptive_line], ...
    {'Frozen','Causally adaptive'},'Orientation','horizontal');
shared_legend.Layout.Tile = 'south';
title(layout,'Frozen versus causally adaptive EEFig');
end

function [frozen_line,adaptive_line] = plotAdaptiveCurve( ...
        ax,frozen,adaptive,color,metric,ylab, ...
        focus_horizon,cfg,plot_title)
frozen = frozen(frozen.Model=="EEFIG offline",:);
hold(ax,'on'); grid(ax,'on');
frozen_line = plot(ax,frozen.HorizonSeconds,frozen.(metric), ...
    '-o','Color',color, ...
    'LineWidth',1.5,'DisplayName','Frozen');
adaptive_line = plot(ax,adaptive.HorizonSeconds,adaptive.(metric), ...
    '--o','Color',color, ...
