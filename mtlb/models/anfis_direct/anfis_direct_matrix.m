...
mu = anfis_direct.norm.mu(:)';
sg = anfis_direct.norm.sigma(:)';
xmin = anfis_direct.norm.x_min(:)';
xmax = anfis_direct.norm.x_max(:)';
...
X_in = min(max(X_in, anfis_direct.norm.x_min(:)'), anfis_direct.norm.x_max(:)'); % Clamp
Xin_n = (X_in - mu) ./ sg;
...
% R dynamics
[A_r_n, b_r_n, ~] = evalfis_mat(anfis_direct.r.mat, Xin_n);
