	...
	*/
	...
	public:
		...
		*/
		...
		/** @brief Searches the nearest neighbor within an interval of indexes, if no start and end input, it performs the search unlimited.
		*/
		int nnSearch(const PointT& query, double* minDist = nullptr, int start=0, int end=std::numeric_limits<int>::max()) const
		{
			int guess;
			double _minDist = std::numeric_limits<double>::max();

			nnSearchRecursive(query, root_, &guess, &_minDist, start, end);
	...
	private:
		...
		*/
		void nnSearchRecursive(const PointT& query, const Node* node, int *guess, double *minDist, int start, int end) const
		{
			if (node == nullptr)
				return;

			if (node->idx < start){
				nnSearchRecursive(query, node->next[1], guess, minDist, start, end);
				return;
			}

			if (node->idx > end){
				nnSearchRecursive(query, node->next[0], guess, minDist, start, end);
				return;
			}
			...
			nnSearchRecursive(query, node->next[dir], guess, minDist, start, end);
